...
The Mac app is a thin Electron shell around a PyInstaller-frozen `cold` engine.
...
```sh
# 1. Freeze the Python engine into dist/cold/ (onedir bundle)
uv run pyinstaller packaging/cold.spec --noconfirm

# 2. Install the Electron shell's dev dependencies
cd app && npm install

# 3. Build the DMG (output: app/release/Cold Storage-<version>-arm64.dmg)
npm run dist
```
...
```sh
uv run pyinstaller packaging/cold.spec --noconfirm   # freeze the engine first
...
- `app/after-pack.js` signs every Mach-O inside the frozen `cold` engine
  **inside-out** with the hardened runtime and `entitlements.mac.plist`, before
  electron-builder seals the outer bundle. With no `COLD_SIGN_IDENTITY` it falls
...
- The PyInstaller spec (`packaging/cold.spec`) bundles the `keyring` macOS
